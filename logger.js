...
const fs = require("fs")
module.exports = (c) => {
    console.log("Caricato tutto il modulo".green);
    try{
      ...
      c.on("channelUpdate", function(oldChannel, newChannel){
        ...
        if(oldChannel.name != newChannel.name){
          send_log(c,
            oldChannel.guild,
            "YELLOW",
            "Canale modificato - NAME",
            `Nome del canale: \`${oldChannel.name}\`\nId del canale: \`${oldChannel.id}\`\n\n`+
            `Nome del canale: \`${newChannel.name}\`\nId del canale: \`${newChannel.id}\``
          )
        }
          ...
          send_log(c,
            ...
            `Nome del canale: \`${oldChannel.name}\`\nId del canale: \`${oldChannel.id}\`\nTipo di canale: \`${types[oldChannel.type]}\`\n\n`+
            `Nome del canale: \`${newChannel.name}\`\nId del canale: \`${newChannel.id}\`\nTipo di canale: \`${types[newChannel.type]}\``
          )
        }
      ...
      c.on("emojiCreate", function (emoji) {
          send_log(c,
            ...
            "GREEN",
            "Emoji creata",
            `EMOJI: ${emoji}\nNome dell'emoji: ${emoji.name}\nId dell'emoji: ${emoji.id}\nURl dell'emoji: ${emoji.url}`,
          )
      });
      ...
      c.on("emojiUpdate", function (oldEmoji, newEmoji) {
          if (oldEmoji.name !== newEmoji.name) {
            send_log(c,
              oldEmoji.guild,
              "ORANGE",
              "Nome dell'emoji modificato",
              `__Emoji: ${newEmoji}__ \n\n**Prima:** \`${oldEmoji.name}\`\n**Dopo:** \`${newEmoji.name}\`\n**Id dell'emoji:** \`${newEmoji.id}\``
            )
          }
      });
      ...
      c.on("guildMemberAdd", function (member) {
          send_log(member.guild,
             ...
             "GREEN",
             "Membro entrato",
             `Membro: ${member.user} (\`${member.user.id}\`)\n\`${member.user.tag}\``,
      ...
      c.on("guildMemberUpdate", function (oldMember, newMember) {
          ...
          if (rolechanged) {
              ...
              let text = `${roleremoved ? `❌ Ruolo rimosso: \n${roleremoved}` : ""}${roleadded ? `✅ Ruolo aggiunto:\n${roleadded}` : ""}`
              send_log(c,
                oldMember.guild,
                `${roleadded ? "GREEN" : "RED"}`,
                "Ruolo del membro cambiato",
                `Membro: ${newMember.user}\nUtente: \`${oldMember.user.tag}\`\n\n${text}`,
                )
          }
...
**Canale : ** <#${message.channel.id}> - *${message.channel.name}*
...
**Autore : ** <@${newMessage.member.user.id}> - *${newMessage.member.user.tag}*
**Data : ** ${newMessage.createdAt}
**Canale : ** <#${newMessage.channel.id}> - *${newMessage.channel.name}*
...
\`\`\``)
      ...
      c.on("roleCreate", function (role) {
          send_log(c,
            ...
            "GREEN",
            "Ruolo creato",
            `Ruolo: <@&${role.id}>\nNome del ruolo: ${role.name}\nId del ruolo: ${role.id}\nColore del ruolo: ${role.hexColor}\nPosizione: ${role.position}`,
            )
      });
      ...
      c.on("roleDelete", function (role) {
          send_log(c,
            ...
            "RED",
            "Ruolo cancellato",
            `Ruolo: <@&${role.id}>\nNome del ruolo: ${role.name}\nId del ruolo: ${role.id}\nColore: ${role.hexColor}\nPozione: ${role.position}`,
            )
      });
...
"MANAGE_EMOJIS": "MANAGE_EMOJIS"
          ...
          if (oldRole.name !== newRole.name) {
              send_log(c,
                oldRole.guild,
                 "ORANGE",
                 "Ruolo del nome creato",
`__ROLE: ${oldRole}__ \n\n**Prima:** \`${oldRole.name}\`
**Dopo:** \`${newRole.name}\`
**Id del ruolo:** \`${newRole.id}\`
`)
          }
            ...
            send_log(c,
              ...
              "ORANGE",
              "ROLE COLOR CHANGED",
              `__ROLE: ${newRole}__ \n\n**Prima:** \`${oldRole.color.toString(16)}\`
            **Dopo:** \`${newRole.color.toString(16)}\`
            **id del ruolo:** \`${newRole.id}\``)
            ...
            send_log(c,
              oldRole.guild,
              "RED",
              "Permesso del ruolo cambiato",
  `__ROLE: ${newRole}__ \n
**Controlla i premessi cambiati**
Vecchio permesso: ${/*perms[String(oldRole.permissions.bitfield)]*/oldRole.permissions.bitfield}
Nuovo permesso: ${/*perms[String(newRole.permissions.bitfield)]*/newRole.permissions.bitfield}
**Id del ruolo** \`${newRole.id}\``)
          }
      });
      ...
      c.on("userUpdate", function (oldUser, newUser) {
          if(oldUser.username !== newUser.username){
              send_log(newUser.guild,
                c,
                "BLACK",
                "Nickname dell'utente cambiato",
                `Membro: ${newUser}\nVecchio nickname: \`${oldUser.username}\`\nNuovo nickname: \`${newUser.username}\` `,
                )
          }
      });
...
async function send_log(c, guild, color, title, description, thumb){
  try {
    ...
    .setFooter(guild.name + " | Creato da: kirito <3", guild.iconURL({format: "png"}))
    //GET THE CHANNEL
